...
# A very limited, hardcoded mapping for common company names to tickers
# This is NOT a comprehensive or dynamic lookup but serves as an example.
# Users will still need to know or find the correct ticker.
COMPANY_TICKER_MAP = {
    "APPLE": "AAPL",
    "GOOGLE": "GOOGL",
    "MICROSOFT": "MSFT",
...
class StreamlitFinancialAnalyzer:
    ...
    def get_financial_summary(_self, ticker_symbol: str) -> Tuple[Optional[Dict], Optional[Dict], Optional[pd.DataFrame]]:
        ...
        try:
            ...
            if history.empty:
                st.warning(f"No historical data found for {ticker_symbol}. This might be due to an incorrect ticker, no available data, or a delisted company. Try checking the ticker on Yahoo Finance.")
                return None, None, None

            # Get basic info with comprehensive error handling
            info = ticker.info
            
            # Check if info is empty or doesn't contain essential data (e.g., longName)
            if not info or not info.get("longName"):
                st.warning(f"Could not retrieve full company information for {ticker_symbol}. Data might be limited or the ticker is invalid. Displaying available data.")
                # Proceed with partial data if possible, or return None if critical info is missing
                if not history.empty and history['Close'].iloc[-1]:
                    # Create a minimal financials dict to avoid errors downstream
                    financials = {
                        "company_name": ticker_symbol.upper(), # Fallback to ticker as name
                        "ticker": ticker_symbol.upper(),
                        "current_price": round(history['Close'].iloc[-1], 2),
                        "analysis_date": _self.current_date.strftime('%Y-%m-%d'),
                        "data_period": f"{start_date} to {end_date}",
                        # Populate other fields as N/A or with available info
                        ...
                        # ... other fields set to "N/A" as needed
                    }
                else:
                    return None, None, None # If no history and no info, return None
            else:
                financials = {
            ...
            st.error(f"Error getting financial data for {ticker_symbol}: {str(e)}. Please ensure the ticker symbol is correct. For Indian stocks, try adding '.NS' (NSE) or '.BO' (BSE) suffix.")
            return None, None, None
    ...
    def calculate_advanced_technical_indicators(self, history: pd.DataFrame) -> Dict:
        ...
        try:
            ...
            if len(close_prices) >= 14:
                ...
                # Use small epsilon to avoid ZeroDivisionError
                rs_denominator = loss
                rs = gain / rs_denominator.replace(0, np.nan) # Replace 0 with NaN to handle division by zero safely
                rsi = 100 - (100 / (1 + rs))
            ...
            if len(history) >= 14:
                ...
                k_percent = 100 * ((close_prices - lowest_low) / k_percent_denominator.replace(0, np.nan)) if (k_percent_denominator != 0).all() else pd.Series([np.nan]*len(close_prices), index=close_prices.index)
    ...
    def generate_investment_summary(self, financials: Dict, technical_indicators: Dict) -> Dict:
        ...
        try:
            ...
            # Debt analysis - Note: debt_to_equity_ratio is not directly pulled from info currently
            # You might need to calculate this from totalDebt and totalStockholderEquity
            total_debt = financials.get("total_debt", "N/A")
            shareholders_equity = financials.get("shareholders_equity", "N/A")

            # Remove non-numeric characters and convert to float for comparison
            if isinstance(total_debt, str) and total_debt.startswith('$'):
                total_debt_val = float(total_debt.replace('$', '').replace('T', 'e12').replace('B', 'e9').replace('M', 'e6').replace('K', 'e3'))
            elif isinstance(total_debt, (int, float)):
                total_debt_val = total_debt
            else:
                total_debt_val = None

            if isinstance(shareholders_equity, str) and shareholders_equity.startswith('$'):
                shareholders_equity_val = float(shareholders_equity.replace('$', '').replace('T', 'e12').replace('B', 'e9').replace('M', 'e6').replace('K', 'e3'))
            elif isinstance(shareholders_equity, (int, float)):
                shareholders_equity_val = shareholders_equity
            else:
                shareholders_equity_val = None

            if total_debt_val is not None and shareholders_equity_val is not None and shareholders_equity_val != 0:
                debt_equity = total_debt_val / shareholders_equity_val
                if debt_equity < 0.5:
                ...
                # Add a message if debt to equity cannot be calculated
                summary["key_concerns"].append("Debt to Equity ratio could not be calculated (missing data or zero equity).")
...
def display_financial_data(financials: Dict, technical_indicators: Dict, investment_summary: Dict, history: pd.DataFrame, analyzer: StreamlitFinancialAnalyzer):
    ...
    with st.expander("\U0001F4CB Valuation & Ratios"):
        ...
        display_section_data("Financial Ratios & Stock Info", {
            # Note: current_ratio, quick_ratio, debt_to_equity_ratio are not directly retrieved from yf.info in your original script.
            # They would need to be calculated from balance sheet items if desired.
    ...
    with st.expander("\U0001F3AF Investment Summary & Analyst Views"):
        ...
        if investment_summary:
            ...
            # Ensure target_mean_price is correctly formatted for display
            target_mean_price = investment_summary['analyst_info'].get('target_mean_price', 'N/A')
            if isinstance(target_mean_price, (int, float)):
                st.write(f"• Target Mean Price: ${target_mean_price:.2f}")
            else:
                st.write(f"• Target Mean Price: {target_mean_price}")
...
def display_comparison_table(company1_data: Dict, company2_data: Dict):
    ...
    for metric_name, key, source in metrics_to_compare:
        ...
        def format_value_for_comparison(val, key):
            try:
                if key in ["current_price", "52_week_high", "52_week_low", "target_price"]:
                    if isinstance(val, (int, float)):
                        return f"${float(val):.2f}"
                    return val
                elif any(k in metric_name for k in ["Yield", "Growth", "Margin", "Return"]): # Added "Return" to catch ROE/ROA
                    if isinstance(val, str) and '%' in val:
                        return val # Already formatted as percentage
                    if isinstance(val, (int, float)):
                        return f"{float(val):.2f}%"
                    return val
                elif key == "macd":
        ...
        # Special handling for large numbers in market_cap, revenue, net_income, EBITDA
        if key in ["market_cap", "revenue", "net_income", "EBITDA"]:
            # These are already formatted by format_large_number in get_financial_summary
            # so no need for further formatting here, just ensure they are strings.
            pass
        else:
            val1 = format_value_for_comparison(val1, key)
            val2 = format_value_for_comparison(val2, key)
...
def get_ticker_from_input(user_input: str) -> str:
    """Attempts to resolve user input (company name or ticker) to a ticker symbol."""
    user_input_upper = user_input.strip().upper()

    # 1. Check direct match in hardcoded map
    if user_input_upper in COMPANY_TICKER_MAP:
        return COMPANY_TICKER_MAP[user_input_upper]
    
    # 2. Check for partial matches (very basic, for illustrative purposes)
    for company_name, ticker in COMPANY_TICKER_MAP.items():
        if user_input_upper in company_name:
            return ticker

    # 3. Assume it's already a ticker if not found in map
...
def main():
    ...
    company1_raw_input = st.sidebar.text_input(
        "Enter Company Name or Ticker Symbol (e.g., Apple, RELIANCE.NS)", 
        value=st.session_state.get('company1_raw_value', "AAPL"), # Persist value
        key="company1_input_widget",
        help="Type a company name or its ticker. For Indian stocks, append '.NS' for NSE or '.BO' for BSE (e.g., RELIANCE.NS, TCS.BO)."
    )
    st.session_state.company1_raw_value = company1_raw_input # Update session state on input
    
    # Resolve the input to a ticker
    ticker1_input = get_ticker_from_input(company1_raw_input)
    ...
    sample_tickers1 = ["AAPL", "GOOGL", "MSFT", "TSLA", "RELIANCE.NS", "TCS.NS", "HDFCBANK.NS"] # Added Indian samples
    selected_sample1 = st.sidebar.selectbox("Select Ticker 1 Sample:", [""] + sample_tickers1, key="sample_select1")
    
    if selected_sample1 and selected_sample1 != company1_raw_input: # Only update if a new sample is chosen
        st.session_state.company1_raw_value = selected_sample1
    ...
    if st.session_state.compare_mode:
        ...
        company2_raw_input = st.sidebar.text_input(
            "Enter Company Name or Ticker Symbol 2 (for comparison)", 
            value=st.session_state.get('company2_raw_value', ""), # Persist value
            key="company2_input_widget",
            help="Type a company name or its ticker. For Indian stocks, append '.NS' for NSE or '.BO' for BSE (e.g., TSLA, TCS.BO)."
        )
        st.session_state.company2_raw_value = company2_raw_input # Update session state on input

        # Resolve the input to a ticker
        ticker2_input = get_ticker_from_input(company2_raw_input)
        ...
        st.sidebar.markdown("#### Or choose from samples:")
        sample_tickers2 = ["TSLA", "NFLX", "GOOG", "AMZN", "ICICIBANK.NS", "INFY.NS"] # Different set for variety, added Indian samples
        selected_sample2 = st.sidebar.selectbox("Select Ticker 2 Sample:", [""] + sample_tickers2, key="sample_select2")
        
        if selected_sample2 and selected_sample2 != company2_raw_input:
            st.session_state.company2_raw_value = selected_sample2
